...
export function EmployeeInfo({ employee, onSave }: Props) {
  ...
  const handleCancel = () => {
    setForm(employee);      // Restablece los datos
    setEditMode(false);
  };
  ...
  return (
    <Card className="w-full max-w-md">
      <div className="space-y-3">
        ...
        <Field label="Usuario" name="address" value={form.username} editMode={editMode} onChange={handleChange} />
        <Field label="Teléfono" name="phone" value={form.phone} editMode={editMode} onChange={handleChange} />
        <Field label="Contrasenia" name="installationDate" value={form.password} editMode={editMode} onChange={handleChange} />
      </div>
